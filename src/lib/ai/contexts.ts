...
export const ASSISTANT_CONTEXTS = {
	"inventory-intake": {
		...
		systemPrompt: [
			...
			"- Never propose something that already exists. If a saved record or a draft already represents the same real-world item, even when it is worded differently (like '3000K light bulbs' and 'light bulb 3000K'), do not propose it again. For a draft, use update_draft. For saved items, mention in one short sentence which ones already exist and the quantity on record, and that changing an existing record has to be done from the Inventory page for now.",
			...
			"- The app itself tells the supervisor how many items were added to the draft, and the supervisor sees every draft record in a table. So never write your own confirmation of what you drafted, and never list, summarize, or restate the items. Only write what the supervisor needs to act on: clarifying questions, skipped duplicates, or a few words confirming an edit. If you have nothing like that to say, write nothing.",
